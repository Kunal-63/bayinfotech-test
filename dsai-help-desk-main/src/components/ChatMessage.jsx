...
const ChatMessage = ({ message, isUser = false, source, confidence, sentiment, type, agentName, agentTier, isTyping = false, guardrail, kbReferences }) => {
    ...
    <MessageBubble isUser={isUser}>
      ...
      <Box sx={{ flex: 1 }}>
        <BubbleContent isUser={isUser} isAgent={!!agentName} isGuardrail={type === 'guardrail'}>
            ...
            <Box sx={{ mt: 1, display: 'flex', flexWrap: 'wrap', gap: 0.5 }}>
              {kbReferences.map((ref, idx) => (
                <SourceChip
                  ...
                  label={`KB: ${ref.title}`}
                  size="small"
                  title={ref.excerpt} // Show excerpt on hover
                />
              ))}
              {confidence && (
                <ConfidenceChip
          ...
          {/* Legacy Source attribution (fallback) */}
          {!isUser && source && !kbReferences && type !== 'guardrail' && (
            <Box sx={{ mt: 1, display: 'flex', flexWrap: 'wrap', gap: 0.5 }}>
              <SourceChip
                label={`Source: ${source}`}
                size="small"
          ...
          {/* Sentiment Indicator */}
          {!isUser && sentiment && sentiment.sentiment !== 'neutral' && (
