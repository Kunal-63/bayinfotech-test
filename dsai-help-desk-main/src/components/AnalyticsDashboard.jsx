...
import {
  ...
  ticketVolumeTrend as mockTicketVolumeTrend,
  topIssueCategories,
  predictiveAlert,
  systemPerformance,
  pcteMetrics,
...
const AnalyticsDashboard = () => {
  ...
  const fetchMetrics = async () => {
    setLoading(true);
    try {
      const [summaryRes, trendsRes, deflectionRes, guardrailsRes] = await Promise.all([
        metricsService.getSummary(),
        metricsService.getTrends(),
        metricsService.getDeflection(),
        metricsService.getGuardrails(),
      ]);

      if (summaryRes.success) setSummary(summaryRes.data);
      if (trendsRes.success) setTrends(trendsRes.data);
      if (deflectionRes.success) setDeflection(deflectionRes.data);
      if (guardrailsRes.success) setGuardrails(guardrailsRes.data);

      // If any failed, we might want to set error, but for now we just log it
      if (!summaryRes.success || !trendsRes.success) {
        console.warn('Some metrics failed to load');
      }
    } catch (err) {
      console.error('Failed to load metrics:', err);
      setError('Failed to load real-time metrics. Showing cached/mock data where available.');
    } finally {
      setLoading(false);
    }
  };

  // Merge real data with mock data for display
  const kpiMetrics = {
    ...mockKpiMetrics,
    ticketsResolved: summary ? {
      ...mockKpiMetrics.ticketsResolved,
      value: summary.total_tickets,
      label: 'Total Tickets',
    } : mockKpiMetrics.ticketsResolved,
    avgResolutionTime: summary ? {
      ...mockKpiMetrics.avgResolutionTime,
      value: Math.round(summary.avg_resolution_time / 60), // Convert seconds to minutes
      unit: 'm',
    } : mockKpiMetrics.avgResolutionTime,
    aiAccuracy: summary ? {
      ...mockKpiMetrics.aiAccuracy,
      value: Math.round(summary.avg_satisfaction * 20), // Convert 0-5 scale to percentage approx or just use as is if backend returns %
    } : mockKpiMetrics.aiAccuracy,
    escalationRate: deflection ? {
      ...mockKpiMetrics.escalationRate,
      value: (100 - deflection.deflection_rate * 100).toFixed(1),
    } : mockKpiMetrics.escalationRate,
  };

  const ticketVolumeData = trends.length > 0 ? trends.map(t => ({
    date: t.date,
    tickets: t.tickets,
  })) : mockTicketVolumeTrend;
  ...
  const lineChartData = {
    ...
    datasets: [
      {
        label: 'Ticket Volume',
        data: ticketVolumeData.map(d => d.tickets),
        borderColor: '#D4AF37',
  ...
  return (
    <Box sx={{ p: { xs: 2, sm: 3, md: 4 }, bgcolor: '#1a1a1a', minHeight: '100vh', width: '100%' }}>
      ...
      {loading ? (
        ...
        <>
          {error && (
            <Alert severity="warning" sx={{ mb: 4, bgcolor: '#2A2417', color: '#E0E0E0', border: '1px solid #FF9500' }}>
              {error}
            </Alert>
          )}
          ...
          <Grid container spacing={3} sx={{ mb: 4, width: '100%' }} data-testid="charts-grid">
            ...
            <Grid item xs={12} lg={4} data-testid="chart-card">
              <Paper
                sx={{
                  p: 3,
                  backgroundColor: '#242424',
              ...
              >
                ...
                <Box sx={{ mt: 2, display: 'flex', gap: 2, flexWrap: 'wrap' }}>
                  <Typography variant="caption" sx={{ color: '#999999' }}>
                    Total Tickets (30 days): {ticketVolumeData.reduce((sum, d) => sum + d.tickets, 0).toLocaleString()}
                  </Typography>
                  <Typography variant="caption" sx={{ color: '#999999' }}>
                  ...
                  <Box sx={{ display: 'flex', flexDirection: 'column', gap: 2 }}>
                    ...
                    <Box>
                      <Box sx={{ display: 'flex', justifyContent: 'space-between', mb: 0.5 }}>
                        ...
                        <Typography variant="body2" sx={{ color: '#4A7C59', fontWeight: 'bold' }}>
                          {deflection ? (deflection.deflection_rate * 100).toFixed(1) : '59.5'}%
                        </Typography>
                      </Box>
                      <LinearProgress
                        variant="determinate"
                        value={deflection ? deflection.deflection_rate * 100 : 59.5}
                        sx={{ height: '8px', backgroundColor: '#333333', '& .MuiLinearProgress-bar': { backgroundColor: '#4A7C59' } }}
                      />
                    </Box>
                    <Box>
                      <Box sx={{ display: 'flex', justifyContent: 'space-between', mb: 0.5 }}>
                        ...
                        <Typography variant="body2" sx={{ color: '#FF4444', fontWeight: 'bold' }}>
                          {deflection ? (100 - deflection.deflection_rate * 100).toFixed(1) : '40.5'}%
                        </Typography>
                      </Box>
                      <LinearProgress
                        variant="determinate"
                        value={deflection ? 100 - deflection.deflection_rate * 100 : 40.5}
                        sx={{ height: '8px', backgroundColor: '#333333', '& .MuiLinearProgress-bar': { backgroundColor: '#FF4444' } }}
                      />
                    </Box>
                  </Box>
                </Paper>
              ...
              {chatbotPredictiveInsights.map((insight, idx) => (
                <Alert
                  ...
                  sx={{
                    ...
                    border: `1px solid ${insight.severity === 'warning' ? '#FF9500' :
